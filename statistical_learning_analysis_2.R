...
sleep_data <- read.csv("sleep_ema_data.csv")
...
dev.off()
# These don't look so bad, but we have to perform data cleaning on the text
...
#dev.new()
#textplot(fitca_trigger$col.scores[,1], fitca_trigger$col.scores[,2], words = rownames(fitca_trigger$col.scores), cex = 0.7,
#         ylim = c(-2,2), asp = 1)
#abline(h = 0, v = 0, lty = 2, col = "gray")
# This won't run, I'm not sure why
...
# Hmmm, not sure if these words really match the emotion words?
...
# Ok, sort of interesting! 



# Prediction

# Let's use jealousy_data$ybocs_total as our response variable, which is a summary measure of 
# retroactive jealousy severity. We'll use the intrusive thought content free response variable
# as our predictor, since we might expect this to relate most strongly to overall jealousy scores. 
...
# 57% accuracy.. this feels like a poor prediction
...
# Ok, so we can see that "jealousy" had the highest positive impact in our regression, which makes a lot of sense,
# since our predictor is jealousy severity. "Thought" had a slight negative impact, which I think means that the more
# someone wrote "thought", the smaller jealousy score they had, which also makes sense. 
